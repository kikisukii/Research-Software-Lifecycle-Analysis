...
# Output to a dedicated test folder to avoid mixing with 8w plots
...
# Core change: use WEEKLY column names
METRICS = [
...
def infer_latest_assignments() -> Path:
    """Pick the newest 05b assignments file under v2_data/05_b_apply/."""
...
def infer_stamp_from_name(p: Path) -> str:
    """Extract timestamp like 20241124_235959 from filename."""
...
def smooth_series(arr: np.ndarray, window: int) -> np.ndarray:
    """Centered rolling mean; keeps peaks aligned (no lag when center=True)."""
    if window is None or window <= 1:
        return arr.astype(float)
    s = pd.Series(arr.astype(float))
    # center=True is important to avoid phase lag of peaks
    return s.rolling(window=window, center=True, min_periods=1).mean().values


def dedupe_by_week(df: pd.DataFrame) -> pd.DataFrame:
    """Ensure at most one row per (repo, week_unix)."""
...
def build_segments(week_unix: np.ndarray, stages: np.ndarray):
    """Turn week/stage arrays into continuous (start, end, stage) segments for axvspan."""
    segs = []
    if len(week_unix) == 0:
        return segs
    start = 0
...
def plot_repo(df_repo: pd.DataFrame, repo: str, stamp: str, outdir: Path, smooth: int = 5):
    ...
    if not required_cols.issubset(df_repo.columns):
        print(f"[ERR] {repo}: Missing WEEKLY columns. Please re-run steps 03 and 05b.")
        return
    ...
    # Legend assembled from STAGE_COLORS and any unseen stage names
    handles = []
...
def main():
    ...
    # [Change] Default smoothing window set to 10 for weekly data
    ap.add_argument("--smooth", type=int, default=10, help="Smoothing window")
    args = ap.parse_args()
    ...
    if missing:
        print(f"[CRITICAL] Input file is missing WEEKLY columns: {missing}")
        print("Please re-run step 03 (with code updates) -> 05b.")
        return

    # Normalize Baseline naming
